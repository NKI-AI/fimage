...
class FastSlideSourceTest : public ::testing::Test {
 protected:
  void SetUp() override {
    // Use existing test SVS files
    test_file_ = "../../checkerboard.svs";
    test_file_cmu_ = "../../CMU-2.svs";
  }

  std::string test_file_;
  std::string test_file_cmu_;
};

// Test FastSlideSource construction and basic properties
TEST_F(FastSlideSourceTest, Construction) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }

  auto source = FastSlideSource::Create(test_file_);
...
// Test FastSlideSource with non-existent file
TEST_F(FastSlideSourceTest, NonExistentFile) {
  EXPECT_THROW(FastSlideSource::Create("non_existent.svs"), std::runtime_error);
}

// Test level count and metadata
TEST_F(FastSlideSourceTest, LevelMetadata) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
  ...
  // Test channel count (constant across all levels)
  int channels = source.GetNumChannels();
  EXPECT_GT(channels, 0) << "Invalid number of channels";
  EXPECT_LE(channels, 4) << "Too many channels";
...
TEST_F(FastSlideSourceTest, InvalidLevel) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
...
TEST_F(FastSlideSourceTest, MicronsPerPixel) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
...
TEST_F(FastSlideSourceTest, LevelViewCreation) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
...
TEST_F(FastSlideSourceTest, GetTileFromLevelView) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
...
TEST_F(FastSlideSourceTest, LevelSwitching) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
...
TEST_F(FastSlideSourceTest, IntegrationWithCrop) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
  ...
  Crop<FastSlideLevelView> crop_op(std::move(level_view), 0, 0, crop_size,
                                   crop_size);
...
TEST_F(FastSlideSourceTest, IntegrationWithDownsample) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
  ...
  Downsample<FastSlideLevelView> ds_op(std::move(level_view),
                                       downsample_factor);
...
// Test move semantics
TEST_F(FastSlideSourceTest, MoveSemantics) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }

  auto source1 = FastSlideSource::Create(test_file_);
  ...
  // Move assign
  auto source3 = FastSlideSource::Create(test_file_);
  source3 = std::move(source2);
  EXPECT_EQ(source3.GetLevelCount(), level_count);
}

// Test with multiple slides
TEST_F(FastSlideSourceTest, MultipleSlides) {
  if (!std::filesystem::exists(test_file_) ||
      !std::filesystem::exists(test_file_cmu_)) {
    GTEST_SKIP() << "Test SVS files not found";
  }

  auto source1 = FastSlideSource::Create(test_file_);
  auto source2 = FastSlideSource::Create(test_file_cmu_);
...
TEST_F(FastSlideSourceTest, LevelViewOutlivesSource) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }

  // Create a level view from a temporary source
  FastSlideLevelView view = [this]() {
    auto source = FastSlideSource::Create(test_file_);
    return source.LevelView(0);
  }();  // source is destroyed here, but view should keep reader alive
...
TEST_F(FastSlideSourceTest, TemporarySourceLevelView) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }

  // This pattern used to be unsafe, but is now safe with shared ownership
  auto view = FastSlideSource::Create(test_file_).LevelView(0);
...
// Test multiple views sharing the same underlying reader
TEST_F(FastSlideSourceTest, MultipleViewsShareReader) {
  if (!std::filesystem::exists(test_file_)) {
    GTEST_SKIP() << "Test SVS file not found: " << test_file_;
  }
  ...
  // Destroy the source - views should keep reader alive via shared ownership
