...
class LodePngSinkTest : public ::testing::Test {
 protected:
  void SetUp() override {}
  ...
  void TearDown() override {
    ...
    std::filesystem::remove("lodepng_sink_test_rgb.png");
    std::filesystem::remove("lodepng_sink_test_rgba.png");
    std::filesystem::remove("lodepng_sink_test_grayscale.png");
  }
};

// Test PNG sink with RGB source
TEST_F(LodePngSinkTest, RgbOutput) {
  MockSource source(50, 40, 3);
  LodePngSink sink = LodePngSink::Create("lodepng_sink_test_rgb.png");

  sink.Render(source);

  EXPECT_TRUE(std::filesystem::exists("lodepng_sink_test_rgb.png"));
  EXPECT_GT(std::filesystem::file_size("lodepng_sink_test_rgb.png"), 0);
}

// Test PNG sink with RGBA source
TEST_F(LodePngSinkTest, RgbaOutput) {
  MockSource source(60, 50, 4);
  LodePngSink sink = LodePngSink::Create("lodepng_sink_test_rgba.png");

  sink.Render(source);

  EXPECT_TRUE(std::filesystem::exists("lodepng_sink_test_rgba.png"));
  EXPECT_GT(std::filesystem::file_size("lodepng_sink_test_rgba.png"), 0);
}

// Test PNG sink with grayscale source
TEST_F(LodePngSinkTest, GrayscaleOutput) {
  MockSource source(30, 25, 1);
  LodePngSink sink = LodePngSink::Create("lodepng_sink_test_grayscale.png");

  sink.Render(source);

  EXPECT_TRUE(std::filesystem::exists("lodepng_sink_test_grayscale.png"));
  EXPECT_GT(std::filesystem::file_size("lodepng_sink_test_grayscale.png"), 0);
}

// Test PNG sink with unsupported channel count
TEST_F(LodePngSinkTest, UnsupportedChannels) {
  MockSource source(50, 40, 2);  // 2 channels not supported
  LodePngSink sink = LodePngSink::Create("test_output.png");

  EXPECT_THROW(sink.Render(source), std::runtime_error);
}

// Test PNG sink with invalid dimensions
TEST_F(LodePngSinkTest, InvalidDimensions) {
...
TEST_F(LodePngSinkTest, LargeImage) {
...
TEST_F(LodePngSinkTest, SmallImage) {
...
TEST_F(LodePngSinkTest, DifferentTileSizes) {
