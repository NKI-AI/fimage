"""Custom pytest entrypoint used by the `src/python/*_test` py_test targets.
...
`bazelisk test @fimage//src/python:*` when fimage is consumed as an external
...
`__file__` and asks pytest to collect from that directory. Because each
py_test only lists its own `*_test.py` in `srcs`, that directory contains
exactly one test module in the sandbox. The same code path works whether
fimage is the root module (standalone build) or loaded via
